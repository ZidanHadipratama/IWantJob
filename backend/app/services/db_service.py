...
class DBService:
    ...
    def delete_job(self, job_id: str) -> bool:
        """Delete a job owned by user_id. Returns True if deleted, False if not found."""
        result = (
            self.client.table("jobs")
            .delete()
            .eq("id", job_id)
    ...
    def save_tailored_resume(self, job_id: str, resume_text: str) -> dict:
        """Insert or update the non-base tailored resume for a specific job."""
        existing = (
            self.client.table("resumes")
            .select("*")
            .eq("job_id", job_id)
        ...
        result = self.client.table("resumes").insert(payload).execute()
        return result.data[0]

...
class TestDBServiceSaveTailoredResume:
    def test_save_tailored_resume_updates_existing_non_base_record(self):
        svc, client = _make_db_service()
        select_chain = client.table.return_value.select.return_value
        select_chain.eq.return_value.eq.return_value.eq.return_value.limit.return_value.execute.return_value.data = [
            {"id": "resume-1"}
        ]
        client.table.return_value.update.return_value.eq.return_value.eq.return_value.execute.return_value.data = [
            {"id": "resume-1", "job_id": "job-abc"}
        ]

        result = svc.save_tailored_resume("job-abc", "{\"sections\": []}")

        client.table.assert_called_with("resumes")
        assert result["id"] == "resume-1"

    def test_save_tailored_resume_inserts_when_missing(self):
        svc, client = _make_db_service()
        select_chain = client.table.return_value.select.return_value
        select_chain.eq.return_value.eq.return_value.eq.return_value.limit.return_value.execute.return_value.data = []
        client.table.return_value.insert.return_value.execute.return_value.data = [
            {"id": "resume-2", "job_id": "job-abc"}
        ]

        result = svc.save_tailored_resume("job-abc", "{\"sections\": []}")

        client.table.assert_called_with("resumes")
        assert result["id"] == "resume-2"
